  ...
  try {
    ...
    if (vFull) {
      ...
      if (employer && employer.telegram_id) {
        ...
        const systemPrompt = "Ты помощник по найму. Сформируй ОДНУ очень короткую фразу (5-7 слов), почему кандидат идеально подходит. Обязательно упомяни, что он живет рядом (в 20 мкр) и имеет нужные навыки для этой вакансии (например, баристы)."
